...
func (k Keeper) RemoveMultiStakingCoinProposal(
	...
	for _, stakingLock := range locks {
		...
		// Incase no delegation or zero share amount
		// Remove multistaking-lock and burn corresponding amount
		if err != nil || unbondAmount.IsZero() {
			// Remove multistaking-lock and burn corresponding amount
