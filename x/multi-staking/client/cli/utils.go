...
func parseAndValidateEVMValidatorJSON(cdc codec.Codec, path string) (evmValidator, error) {
	...
	if v.Amount == "" {
		return evmValidator{}, fmt.Errorf("must specify amount of coins to bond")
	}
	...
	if !common.IsHexAddress(v.ContractAddress) {
		return evmValidator{}, fmt.Errorf("invalid contract address")
	}

	if v.PubKey == nil {
		return evmValidator{}, fmt.Errorf("must specify the JSON encoded pubkey")
	}
	...
	if v.Moniker == "" {
		return evmValidator{}, fmt.Errorf("must specify the moniker name")
	}
	...
	if v.MinSelfDelegation == "" {
		return evmValidator{}, fmt.Errorf("must specify minimum self delegation")
	}
	minSelfDelegation, ok := math.NewIntFromString(v.MinSelfDelegation)
	if !ok {
		return evmValidator{}, errorsmod.Wrap(sdkerrors.ErrInvalidRequest, "minimum self delegation must be a positive integer")
	}
